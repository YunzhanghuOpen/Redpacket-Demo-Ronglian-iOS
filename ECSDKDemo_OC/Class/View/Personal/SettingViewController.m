...
#pragma redpacke_fix? 添加零钱按钮
    UIButton *ChangePurseBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    ChangePurseBtn.frame =CGRectMake(0.0f, aboutBtn.frame.origin.y+excNowNumBtn.frame.size.height+Group_Margin_Heigth, 320.0f, 50.0f);
    [ChangePurseBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    UILabel *ChangePurseLabel = [[UILabel alloc] initWithFrame:CGRectMake(30.0f, 0.0f, 200.0f, 50.0f)];
    ChangePurseLabel.text = @"零钱";
    ChangePurseLabel.backgroundColor = [UIColor clearColor];
    [ChangePurseBtn addSubview:ChangePurseLabel];
    ChangePurseBtn.titleLabel.textAlignment = NSTextAlignmentLeft;
    [ChangePurseBtn setBackgroundImage:[CommonTools createImageWithColor:[UIColor whiteColor]] forState:UIControlStateNormal];
    [ChangePurseBtn addTarget:self action:@selector(ChangePurseBtnClicked) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:ChangePurseBtn];
    
    self.title =@"设置";
    
    ((UIScrollView*)self.view).contentSize = CGSizeMake([UIScreen mainScreen].bounds.size.width, ChangePurseBtn.frame.origin.y+ChangePurseBtn.frame.size.height+Group_Margin_Heigth);
