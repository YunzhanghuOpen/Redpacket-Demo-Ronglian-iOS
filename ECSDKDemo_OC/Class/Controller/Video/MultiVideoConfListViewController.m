...
- (void)loadView
{
    ...
    statusLabel.backgroundColor = [UIColor colorWithRed:0.28 green:0.74 blue:0.49 alpha:1.00];
    ...
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|[statusLabel(==22)]|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(statusLabel)]];
    
    {
        UIButton *noneDataView = [UIButton buttonWithType:UIButtonTypeCustom];
        noneView = noneDataView;
        noneView.userInteractionEnabled = NO;
        [self.view addSubview:noneDataView];
        [noneView setImage:[CommonTools changeImage:[UIImage imageNamed:@"videoConf32"] WithColor:[UIColor blueColor]] forState:UIControlStateNormal];
        [noneView setTitle:@"暂无房间，请先创建一个" forState:UIControlStateNormal];
        [noneView setAttributedTitle:[[NSAttributedString alloc] initWithString:noneView.currentTitle attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15.0f],NSForegroundColorAttributeName:[UIColor colorWithRed:204.0f/255.0f green:204.0f/255.0f blue:204.0f/255.0f alpha:1.0f]}] forState:UIControlStateNormal];
    
        noneView.translatesAutoresizingMaskIntoConstraints = NO;
        [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"|[noneView]|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(noneView)]];
        [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-[noneView]-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(noneView)]];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil) {
        ...
        lock_closedImage.center = CGPointMake(tableView.frame.size.width-44.0f, 20.0f);
    ...
    cell.detailTextLabel.text = [NSString stringWithFormat:@"%@,由%@创建", info, [roomInfo.creator substringFromIndex:fromIndex]];
