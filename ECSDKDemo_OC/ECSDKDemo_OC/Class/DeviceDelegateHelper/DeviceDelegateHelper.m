...
-(void)onSoftVersion:(NSString*)version andUpdateMode:(NSInteger)mode andUpdateDesc:(NSString*)desc{
    ...
    NSComparisonResult result = [version compare:[NSString stringWithFormat:@"%d.%d.%dr",major,minor,micro]];
    if (result == 1) {
...
- (void)onReceiveMessageNotify:(ECMessageNotifyMsg *)message {
    ...
    if (message.messageType == ECMessageNotifyType_DeleteMessage) {
        ...
        NSInteger readCount = [[NSUserDefaults standardUserDefaults] integerForKey:[NSString stringWithFormat:@"%@_%@",isReadMsg.messageId,CellMessageReadCount]];
        if ([isReadMsg.sessionId hasPrefix:@"g"] && unreadCount>0) {
            unreadCount--;
            readCount++;
            [[NSUserDefaults standardUserDefaults] setInteger:unreadCount forKey:[NSString stringWithFormat:@"%@_%@",isReadMsg.messageId,CellMessageUnReadCount]];
            [[NSUserDefaults standardUserDefaults] setInteger:readCount forKey:[NSString stringWithFormat:@"%@_%@",isReadMsg.messageId,CellMessageReadCount]];
