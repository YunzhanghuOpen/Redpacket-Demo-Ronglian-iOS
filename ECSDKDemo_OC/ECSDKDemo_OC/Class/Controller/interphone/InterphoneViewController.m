...
@interface InterphoneViewController () <UIActionSheetDelegate>
{
...
- (void)viewDidLoad {
    ...
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[statusLabel]|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(statusLabel)]];
    
    UIView *noneDataView = [[UIView alloc] init];
    noneDataView.translatesAutoresizingMaskIntoConstraints = NO;
    noneView = noneDataView;
    [self.view addSubview:noneDataView];
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[noneDataView]|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(noneDataView)]];
    [self.view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|[statusLabel(==29)][noneDataView]|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(statusLabel,noneDataView)]];
    ...
    [noneDataView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[text]|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(text)]];
    ...
    [noneDataView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:[startBtn(==136)]" options:0 metrics:nil views:NSDictionaryOfVariableBindings(startBtn)]];
    [noneDataView addConstraint:[NSLayoutConstraint constraintWithItem:startBtn attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:noneDataView attribute:NSLayoutAttributeCenterX multiplier:1 constant:0]];
    
    [noneDataView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-30-[text(==25)]-40-[startBtn(==38)]" options:0 metrics:nil views:NSDictionaryOfVariableBindings(text,startBtn)]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [interphoneIdArray objectAtIndex:indexPath.row];
    return cell;
}
...
- (void)startInterphone:(id)sender {
    [self pushInterphone:0];
//    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@"选择对讲模式" delegate:self cancelButtonTitle:@"取消(0)-默认方式进入" destructiveButtonTitle:nil otherButtonTitles:@"1-无提示音有背景音",@"2-有提示音有背景音",@"3-无提示音无背景音", nil];
//    sheet.actionSheetStyle = UIActionSheetStyleDefault;
//    [sheet showInView:self.view];
}

- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    NSInteger voiceMode = [[actionSheet buttonTitleAtIndex:buttonIndex] integerValue];
    [self pushInterphone:voiceMode];
}

- (void)pushInterphone:(NSInteger)voiceMode {
    //当前正在一个对讲中,需要先退出才可创建
    [DemoGlobalClass sharedInstance].curinterphoneid = nil;
    [[ECDevice sharedInstance].meetingManager exitMeeting];
    ...
    selectView.voiceMode = voiceMode;
